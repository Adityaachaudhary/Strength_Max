...
export default function ExerciseCard(props) {
  ...
  const [setsCompleted, setSetsCompleted] = useState(0); // Fixed typo in state setter name

  function handleSetIncrement() {
    setSetsCompleted((prevSets) => (prevSets + 1) % 6); // Fixed logic to correctly update setsCompleted
  }
    ...
    <div className="p-4 rounded-md flex flex-col gap-4 bg-slate-950 sm:flex-wrap">
      <div className="flex flex-col sm:flex-row sm:items-center sm:flex-wrap gap-x-4">
        <h4 className="text-3xl hidden sm:inline sm:text-4xl md:text-5xl font-semibold text-slate-400">
          {String(i + 1).padStart(2, '0')} {/* Dynamically padded exercise index */}
        </h4>
        <h2 className="capitalize whitespace-nowrap truncate max-w-full text-lg sm:text-xl md:text-2xl flex-1 sm:text-center">
          {exercise.name.replaceAll('_', ' ')} {/* Properly formatted exercise name */}
        </h2>
      ...
      <div className="flex flex-col bg-slate-950 rounded gap-2">
        {exercise.description.split('___').map((val, index) => (
          <div key={index} className="text-sm">
            {val} {/* Dynamically rendered description parts */}
          </div>
        ))}
      </div>
